...
function runLoadFlow() {
    fetch('/run_loadflow', { method: 'POST' })
        .then(response => response.json())
        .then(data => {
            // Display bus voltages
            let voltTable = "<table style='width:100%;border-collapse:collapse;'><tr><th>Bus</th><th>V (pu)</th><th>Angle (deg)</th></tr>";
            data.bus_voltages.forEach(row => {
                voltTable += `<tr>
                    <td style='border:1px solid #ccc;padding:4px;'>${row.bus}</td>
                    <td style='border:1px solid #ccc;padding:4px;'>${row.V.toFixed(4)}</td>
                    <td style='border:1px solid #ccc;padding:4px;'>${row.angle.toFixed(3)}</td>
                </tr>`;
            });
            voltTable += "</table>";
            document.getElementById("voltagesResult").innerHTML = voltTable;
...
// Modal close logic
document.getElementById("closeLoadFlowModal").onclick = function() {
